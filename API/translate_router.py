...
async def sign_to_text_handler(
    ...
    # expected_word를 선택적 파라미터로 설정하여 A타입, B타입 요청을 구분
...
async def analyze_frames(
    ...
    try:
        ...
        last_result = {
            "korean": last_result,
            "english": serialize_result(translator.translate_text(last_result, target_lang="EN-US")),
            "japanese": serialize_result(translator.translate_text(last_result, target_lang="JA")),
            "chinese": serialize_result(translator.translate_text(last_result, target_lang="ZH")),
        }
        return last_result
    except Exception as e:
...
async def translate_latest(request: Request, response: Response, db: Session = Depends(get_db)):
    ...
    # 결과를 가져온 후에는 해당 유저의 Recognizer 상태를 초기화하여 다음 문장을 받을 준비
    try:
        recognizer.reset()
    except Exception as e:
